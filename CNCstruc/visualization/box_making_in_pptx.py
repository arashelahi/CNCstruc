...
def create_presentation(CNC_group):
    ...
    Interior_Image_dir = './tg_data/Images/interior/'
    Exterior_Image_dir = './tg_data/Images/exterior/'

    ppt_dir = './tg_data/Images/'
    ...
    plot_to_box_offset_x = Inches(0.11)
    ...
    for layer_index, layer in enumerate(CNC_group.layer_vec):
        ...
        for chain_iter,chain_number in enumerate(chain_number_vec):
            ...
            if len(chain_number_vec)<=2 or chain_iter == 0 or chain_iter == len(chain_number_vec)-1:
                ...
                file_ext = '_ends'
                plot_file_name = Exterior_Image_dir+'%s_ch%s_tgs%s.png' % (layer, chain_iter if chain_iter==0 else 1 , file_ext)
            else:
                domain = 'interior'
                rgb_color = RGBColor(192, 0, 0)
                file_ext = ''
                plot_file_name = Interior_Image_dir+'%s_ch%s_tgs%s.png' % (layer, chain_iter-1, file_ext)
